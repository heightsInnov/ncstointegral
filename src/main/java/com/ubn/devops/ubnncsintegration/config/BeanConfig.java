...
public class BeanConfig {
	...
	public DataSource dataSource() throws IllegalArgumentException, NamingException {
		...
		if (dbType.contentEquals(USERNAMEPASSWORD)) {
			...
			dataSource = DataSourceBuilder.create().driverClassName(driverclassName).password(password)
	...
	public Docket api() {
		return new Docket(DocumentationType.SWAGGER_2).select()
				.apis(RequestHandlerSelectors.basePackage("com.ubn.devops.ubnncsintegration"))
				.paths(PathSelectors.any()).build().apiInfo(apiInfo())
				.securitySchemes(Arrays.asList(new ApiKey("JWT", HttpHeaders.AUTHORIZATION, "header")));
	}

	private ApiInfo apiInfo() {
		return new ApiInfo("NCS/Union Bank Integration Microservice", "NCS Integration with UBN", "API TOS",
				"Terms of service", new Contact("Devops", "www.unionbankng.com", "customerservice@unionbankng.com"),
