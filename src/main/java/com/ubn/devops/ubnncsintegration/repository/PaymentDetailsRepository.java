...
public class PaymentDetailsRepository {
			...
			if (!result.isEmpty()) {
				...
				if (rspCode.equals("00")) {
					if (result.get("P_DATA") != null) {
						...
						if (!payments.isEmpty()) {
							for(PaymentDetails details:payments) {
								if(details.getPaymentStatus().equalsIgnoreCase(PaymentDetails.PAYED)) {
									model =  details;
									log.info("Payment has already been made");
								}
							}
							if(model==null) {
	...
	public int validateTransactionReference(PaymentProcessRequest request) {
		...
		try {
			...
			call.declareParameters(new SqlParameter("P_TXNREFERENCE", Types.VARCHAR),
					new SqlParameter("P_UNIQUEID", Types.VARCHAR),
					new SqlOutParameter("P_RESPONSEMSG", Types.VARCHAR),
					new SqlOutParameter("P_RESPONSECODE", Types.VARCHAR));
			Map<String, Object> paramSource = new HashMap<>();
			paramSource.put("P_TXNREFERENCE", request.getExternalRef());
			paramSource.put("P_UNIQUEID", request.getUniqueId());
			Map<String, Object> result = call.execute(paramSource);
			if (!result.isEmpty()) {
				String rspCode = result.get("P_RESPONSECODE").toString();
				if (rspCode.equals("00")) {
	...
	public int updateSuccessfulValidation(PaymentProcessRequest request,int validationRspCode,String validationRspMsg) {
		...
		try {
			...
			call.declareParameters(new SqlParameter("P_UNIQUEID", Types.VARCHAR),
	...
	public int updatePaymentWithNCSResponse(TransactionResponse response) {
		int isUpdated = 0;
		log.info("trying to update payment details with transaction response from NCS with details: "+response.toString());
		try {
			SimpleJdbcCall call = new SimpleJdbcCall(jdbcTemplate);
			call.setSchemaName(SCHEMANAME);
			call.setCatalogName(PACKAGENAME);
...
//					.addValue("RESPONSE_TIME", sweepAgent.getResponsetime()).addValue("UIDSS", sweepAgent.getUids())
	...
	@SuppressWarnings("unchecked")
	public PaymentDetails findByUniqueId(String uniqueId) {
		PaymentDetails details = null;
		log.info("==== Trying to find Payment details by uniqueId: "+uniqueId);
		try {
			SimpleJdbcCall call = new SimpleJdbcCall(jdbcTemplate);
			call.setSchemaName(SCHEMANAME);
			call.setCatalogName(PACKAGENAME);
			call.setProcedureName("FINDBYUNIQUEID");
			call.declareParameters(new SqlParameter("P_UNIQUEID", Types.VARCHAR),
					new SqlOutParameter("P_RESPONSEMSG", Types.VARCHAR),
					new SqlOutParameter("P_RESPONSECODE", Types.VARCHAR),
					new SqlOutParameter("P_DATA", Types.REF_CURSOR));
			call.addDeclaredRowMapper("P_DATA", new PaymentDetailsMapper());
			Map<String, Object> paramSource = new HashMap<>();
			paramSource.put("P_UNIQUEID", uniqueId);
			Map<String, Object> result = call.execute(paramSource);
			if(!result.isEmpty()) {
				String rspCode = result.get("P_RESPONSECODE").toString();
				if(rspCode.equals("00")) {
					List<PaymentDetails> paymentDetails = (List<PaymentDetails>) result.get("P_DATA");
					if(!paymentDetails.isEmpty()) {
						details = paymentDetails.get(0);
						log.info("Successfully found details");
					}else {
						log.warn("Unable to find details with uniqueId: "+uniqueId);
					}
				}
			}
		}catch(Exception ex) {
			log.error("Error occured while trying to find payment details with uniqueId: "+uniqueId+" because: "+ex.getMessage(),ex);
		}
		return details;
	}
	
	public StatusResponse getStatus(String uniqueId) {
		StatusResponse response = null;
		log.info("Trying to get status of payment details with unique id: "+uniqueId);
		try {
			SimpleJdbcCall call = new SimpleJdbcCall(jdbcTemplate);
			call.setSchemaName(SCHEMANAME);
			call.setCatalogName(PACKAGENAME);
			call.setProcedureName("GETSTATUS");
			call.declareParameters(new SqlParameter("P_UNIQUEID", Types.VARCHAR),
					new SqlOutParameter("P_RESPONSEMSG", Types.VARCHAR),
					new SqlOutParameter("P_RESPONSECODE", Types.VARCHAR),
			...
			if(!result.isEmpty()) {
				String rspCode = result.get("P_RESPONSECODE").toString();
				if(rspCode.equals("00")) {
					response = new StatusResponse(result.get("P_STATUS").toString(), result.get("P_MESSAGE").toString());
					log.info("Succefully found status using unique id: "+uniqueId+" returning: "+response.toString());
				}else if(rspCode.equals("11")){
					response = new StatusResponse(String.valueOf(ApiResponse.NOT_FOUND), "Not Found");
					log.warn("db error occured while trying to get status with uniqueId: "+uniqueId+" with message: "+result.get("P_RESPONSEMSG").toString());
				}else {
					log.error("db error occured while trying to get status with uniqueId: "+uniqueId+" with message: "+result.get("P_RESPONSEMSG").toString());
					
				}
			}
		}catch(Exception ex) {
			log.error("Error occured while trying to get the payment details using uniqueId: "+uniqueId+" because: "+ex.getMessage(),ex);
		}
		return response;
	}
	
	@SuppressWarnings("unchecked")
	public List<PaymentDetails> findAllPayedAndNcsConfirmed(){
		List<PaymentDetails> details = new ArrayList<>();
		try {
			SimpleJdbcCall call = new SimpleJdbcCall(jdbcTemplate);
			call.setSchemaName(SCHEMANAME);
			call.setCatalogName(PACKAGENAME);
			call.setProcedureName("FINDALLPAIDANDCONFIRMED");
			call.declareParameters(new SqlOutParameter("P_DATA", Types.REF_CURSOR),
					new SqlOutParameter("P_RESPONSECODE", Types.VARCHAR),
					new SqlOutParameter("P_RESPONSEMSG", Types.VARCHAR));
			call.addDeclaredRowMapper("P_DATA", new PaymentDetailsMapper());
			Map<String,Object> result = call.execute();
			if(!result.isEmpty()) {
				if(result.get("P_RESPONSECODE").equals("00")) {
					details = (List<PaymentDetails>) result.get("P_DATA");
				}else{
					log.warn("DB error occured while trying to findallPayedAndNcsResponse with error message: "+result.get("P_RESPONSEMSG").toString());
				}
			}
		}catch(Exception ex) {
			log.error("Error occured while trying findallPayedAndNcsResponse because: "+ex.getMessage(),ex);
		}
		return details;
	}
